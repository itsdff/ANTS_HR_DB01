...
const STATUS_DOT: Record<string, string> = {
  "On Time":"bg-black", "Late":"bg-red-600", "Absent":"bg-red-300",
  "WFH":"bg-gray-700", "Leave":"bg-red-400", "Late Start Request":"bg-red-500",
  "Holiday":"bg-gray-400", "Not Scheduled":"bg-gray-300", "Day Off":"bg-gray-300",
};
...
const parseDMY = (dmy: string): string => {
  try {
    const p = parse(dmy, "dd/MM/yyyy", new Date());
    if (isNaN(p.getTime())) return dmy;
...
export default function HRDashboard() {
  ...
  // Summary month filter: "yyyy-MM" or "all"
  ...
  // ─── Theme classes ─────────────────────────────────────────────────────────
  // Red/white/black: light = white bg, dark = near-black bg; red is the accent in both
  ...
  // Available months in records for summary dropdown
  ...
  const handleEmpCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload=ev=>{
      ...
      const imported=lines.map(line=>{
        ...
        count++;
        return migrateEmployee({ id:Date.now().toString()+count, employeeId:p[0],name:p[1],designation:p[2],department:p[3],bloodGroup:p[4],phone:p[5],email:p[6],companyEmail:p[7],presentAddress:p[8],birthday:p[9],joiningDate:p[10],employmentType:p[11]==="part-time"?"part-time":"full-time",positionType:p[12]==="contractual"?"contractual":"permanent",totalLeaves:parseInt(p[13])||20,remainingLeaves:parseInt(p[13])||20 });
      }).filter(e=>e.name);
      if(imported.length){ setEmployees(prev=>{ const m=new Map(prev.map(e=>[e.employeeId,e])); imported.forEach(e=>m.set(e.employeeId,e)); return Array.from(m.values()); }); alert(`✅ Imported ${imported.length} employees.`); }
  ...
  const handleHolidayCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload=ev=>{
      ...
      (ev.target?.result as string).split("\n").forEach(line=>{ const p=line.split(",").map(s=>s.replace(/"/g,"").trim()); if(p[0]?.match(/^\d{4}-\d{2}-\d{2}$/)) parsed.push({date:p[0],name:p[1]||"Holiday",type:p[2]==="company"?"company":"public",considerAttendance:false}); });
      if(parsed.length){ setHolidays(prev=>{ const m=new Map(prev.map(h=>[h.date,h])); parsed.forEach(h=>m.set(h.date,h)); return Array.from(m.values()); }); alert(`✅ Imported ${parsed.length} holidays.`); }
      else alert("❌ No valid rows.");
    };
    reader.readAsText(file); e.target.value="";
  };

  const downloadCSV=(rows:string[][],filename:string)=>{ const csv=rows.map(r=>r.map(c=>`"${c}"`).join(",")).join("\n"); const blob=new Blob([csv],{type:"text/csv"}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=filename; a.click(); URL.revokeObjectURL(url); };
  const exportAttendanceCSV=()=>{ const rows=[["Date","Employee ID","Name","Designation","Department","Employment","Position","Status","In Time","Note","Late Reason"]]; records.forEach(r=>{ const emp=employees.find(e=>e.id===r.employeeId); rows.push([fmtDate(r.date),emp?.employeeId||"",emp?.name||"",emp?.designation||"",emp?.department||"",emp?.employmentType||"",emp?.positionType||"",r.status,r.inTime,r.note,r.lateReason]); }); downloadCSV(rows,"ants_attendance.csv"); };
  const exportEmployeesCSV=()=>{ const rows=[EMP_CSV_HEADERS]; employees.forEach(e=>rows.push([e.employeeId,e.name,e.designation,e.department,e.bloodGroup,`${e.phoneCode} ${e.phone}`,e.email,e.companyEmail,e.presentAddress,e.birthday,e.joiningDate,e.employmentType,e.positionType,String(e.totalLeaves)])); downloadCSV(rows,"ants_employees.csv"); };
  ...
  const OrgNode=({emp,depth}:{emp:Employee;depth:number})=>{
    ...
    return (
      <div className="flex flex-col items-center">
        <div className={`rounded-xl border p-3 w-40 text-center shadow-sm ${depth===0?darkMode?"bg-red-900/30 border-red-700":"bg-red-50 border-red-300":darkMode?"bg-[#1a1a1a] border-[#2a2a2a]":"bg-white border-gray-200"}`}>
          <div className={`w-8 h-8 rounded-full mx-auto mb-1.5 flex items-center justify-center text-xs font-bold text-white ${depth===0?"bg-red-600":depth===1?"bg-black":"bg-gray-500"}`}>
            {emp.name.split(" ").map(n=>n[0]).join("").slice(0,2).toUpperCase()}
          </div>
          {emp.isCsuite&&<div className="flex justify-center mb-1"><Star size={10} className="text-red-500 fill-red-500"/></div>}
  ...
  const OrganogramView=()=>{
    ...
    return (
      <div className={`${cardCls} p-6 overflow-auto`}>
        <h3 className={`text-base font-semibold mb-6 flex items-center gap-2 ${txt}`}><GitBranch size={16} className="text-red-600"/> Organisation Chart</h3>
        <div className="flex gap-8 items-start justify-center flex-wrap min-w-max mx-auto pb-4">{roots.map(e=><OrgNode key={e.id} emp={e} depth={0}/>)}</div>
        <p className={`text-xs mt-4 text-center ${txtSub}`}>Set "Reports To" in each employee profile to build the hierarchy.</p>
      </div>
    );
  };
  ...
  const fi=(label:string,key:keyof Omit<Employee,"id">,type="text",opts?:string[])=>(
    <div>
      ...
      {opts
        ?<select value={empForm[key] as string} onChange={e=>setEmpForm(f=>({...f,[key]:e.target.value}))} className={inputCls}><option value="">Select…</option>{opts.map(o=><option key={o}>{o}</option>)}</select>
  ...
  const sectionTitle=(label:string)=>(
    <p className={`text-xs font-bold uppercase tracking-wider mb-3 ${txtMuted}`}>{label}</p>
  );
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${bg} ${txt}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        {/* ── Stats bar — always today ─────────────────────────────────────── */}
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            {label:"Total",   value:todayStats.total,   Icon:Users,        accent:"border-l-black"},
            {label:"Present", value:todayStats.present, Icon:CheckCircle2, accent:"border-l-red-600"},
            {label:"Late",    value:todayStats.late,    Icon:Clock,        accent:"border-l-red-400"},
            {label:"WFH",     value:todayStats.wfh,     Icon:Laptop,       accent:"border-l-gray-400"},
            {label:"On Leave",value:todayStats.onLeave, Icon:XCircle,      accent:"border-l-red-300"},
            {label:"Absent",  value:todayStats.absent,  Icon:AlertTriangle,accent:"border-l-gray-300"},
          ].map(({label,value,Icon,accent})=>(
            <div key={label} className={`${cardCls} p-4 flex items-center gap-3 border-l-4 ${accent}`}>
              <Icon size={18} className={darkMode?"text-gray-300":"text-gray-700"}/>
              <div>
        ...
        <p className={`text-xs -mt-4 ${txtSub}`}>Today's summary — {fmtDate(todayStr)}</p>
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              <div className="flex items-center gap-3 flex-wrap">
                <h2 className={`text-lg font-bold flex items-center gap-2 ${txt}`}>
                  ...
                  <span className={`text-xs font-normal px-2 py-0.5 rounded-full border ${darkMode?"border-[#2a2a2a] text-gray-400":"border-gray-200 text-gray-500"}`}>{employees.length} total · {activeEmployees.length} active</span>
                </h2>
                <div className={`flex rounded-lg p-1 gap-1 ${darkMode?"bg-[#141414] border border-[#2a2a2a]":"bg-gray-100"}`}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button onClick={downloadEmpTemplate} className={btnGhost}><Download size={13}/> Template</button>
                  ...
                  <div key={dept}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                      {emps.map(emp=>{
                        ...
                        return (
                          <div key={emp.id}
                            className={`${cardCls} p-4 flex flex-col items-center text-center gap-2 transition-all hover:shadow-md hover:border-red-300 cursor-default ${isInactive?"opacity-50":""}`}>
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0 relative ${isInactive?"bg-gray-400":emp.isCsuite?"bg-red-600":"bg-black"}`}>
                              {emp.name.split(" ").map(n=>n[0]).join("").slice(0,2).toUpperCase()}
                            ...
                            <div className="min-w-0 w-full">
                              ...
                              {isInactive&&<span className={`text-xs px-1.5 py-0.5 rounded-full border ${border} ${txtMuted}`}>Inactive</span>}
            ...
            {attendanceView==="summary"&&(
              <div className={`${cardCls} p-6`}>
                ...
                <p className={`text-xs mb-4 ${txtMuted}`}>
                  Showing: <strong>{summaryMonthLabel}</strong> · Leave balances show yearly totals (all time).
                </p>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className={`border-b text-xs uppercase ${darkMode?"border-[#2a2a2a] text-gray-500":"border-gray-100 text-gray-400"}`}>
                        ...
                        <th className="py-3 text-center">Leaves Left (YTD)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.entries(groupByDept(employees)).sort(([a],[b])=>a.localeCompare(b)).flatMap(([dept,emps])=>[
                        <tr key={`dept-${dept}`}>
                          <td colSpan={10} className={`py-2 text-xs font-bold uppercase tracking-wider ${darkMode?"text-gray-500":"text-gray-400"}`}>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
                <div className={`${cardCls} xl:col-span-3 p-6`}>
                  <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <label className={`text-xs font-medium ${txtMuted}`}>Late after</label>
                              ...
                              <tr key={emp.id} className={`border-b last:border-0 ${darkMode?"border-[#1a1a1a]":"border-gray-50"}`}>
                                ...
                                <td className="py-3">
                                  <div className="flex items-center justify-center gap-1 flex-wrap">
                                    ...
                                    {showClear&&(
                                      <button onClick={()=>clearEntry(emp.id)} title="Clear entry"
                                        className={`p-1.5 rounded-lg hover:scale-110 transition-all ${darkMode?"hover:bg-[#2a2a2a] text-gray-500":"hover:bg-gray-100 text-gray-400"}`}><Ban size={15}/></button>
                                    )}
                                  </div>
                                </td>
                                <td className="py-3 text-center">
                ...
                <div className="space-y-4">
                  {(upcomingBirthdays.length>0||upcomingHolidays.length>0)&&(
                    <div className={`${cardCls} p-5`}>
                      <h3 className={`text-sm font-semibold mb-3 flex items-center gap-2 ${txt}`}><Bell size={14} className="text-red-500"/> Upcoming Alerts</h3>
                      <div className="space-y-2">
                        {upcomingBirthdays.map(emp=>(
                          <div key={emp.id} className={`flex items-center gap-2 p-2 rounded-lg border ${darkMode?"border-[#2a2a2a] bg-[#1a1a1a]":"border-red-100 bg-red-50"}`}>
                        ...
                        {upcomingHolidays.map(h=>(
                          <div key={h.date} className={`flex items-center gap-2 p-2 rounded-lg border ${darkMode?"border-[#2a2a2a] bg-[#1a1a1a]":"border-gray-200 bg-gray-50"}`}>
                  ...
                  <div className={`${cardCls} p-5`}>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      <button onClick={()=>setShowHolidayForm(true)} className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-medium bg-red-600 hover:bg-red-700 text-white">
                        <Plus size={11}/> Add
                      </button>
                      <input ref={holidayCsvRef} type="file" accept=".csv" className="hidden" onChange={handleHolidayCSV}/>
                      <button onClick={downloadHolidayTemplate} className={btnGhost}><Download size={11}/> Template</button>
                      <button onClick={()=>holidayCsvRef.current?.click()} className={btnGhost}><Upload size={11}/> Import CSV</button>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className={`${cardCls} w-full max-w-xs p-6 shadow-2xl`}>
            ...
            {(()=>{ const emp=employees.find(e=>e.id===lateStartModal.empId); const used=emp?getLateStartCount(emp.id,selectedDate.slice(0,7)):0; const limit=emp?.lateStartLimit??2;
              return used>=limit?(<div className="flex items-center gap-2 p-2 rounded-lg mb-4 bg-red-50 border border-red-200"><AlertTriangle size={14} className="text-red-500 shrink-0"/><p className="text-xs text-red-700">Limit reached: {used}/{limit} this month. You can still proceed.</p></div>):null;
            })()}
      ...
      {lateStartWarning&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className={`${cardCls} w-full max-w-sm p-6 shadow-2xl`}>
            ...
            <p className={`text-sm mb-5 ${txtMuted}`}><strong>{lateStartWarning.empName}</strong> has used <strong>{lateStartWarning.used}</strong> of <strong>{lateStartWarning.limit}</strong> allowed late starts this month. Entry recorded.</p>
      ...
      {holidayWarning&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className={`${cardCls} w-full max-w-sm p-6 shadow-2xl`}>
            ...
            <p className={`text-sm mb-5 ${txtMuted}`}><strong>{fmtDate(selectedDate)}</strong> is a holiday (<strong>{getHoliday(selectedDate)?.name}</strong>) with attendance tracking off. Won't deduct from leave. Proceed?</p>
            ...
            <div className="p-6 space-y-5">
                  ...
                  <div className="sm:col-span-2">
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      <div>
                        ...
                        <div className={`text-xs ${txtMuted}`}>Enables "Clear Entry" button in attendance. Shown with ★ in all views.</div>
                      </div>
                    </label>
                  </div>
                </div>
              ...
              <div>{sectionTitle("Employment")}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {fi("Joining Date","joiningDate","date")} {fi("Cessation Date","cessationDate","date")}
