...
// ─── Types ────────────────────────────────────────────────────────────────────
type AttendanceStatus = "On Time" | "Late" | "Absent" | "WFH" | "Leave" | "Late Start Request" | "Holiday" | "Not Scheduled";
type MainTab = "employees" | "attendance";
type EmpView = "cards" | "organogram";
...
interface Employee {
  ...
  workingHours: string; // free text e.g. "10:00-14:00" or "4 hours"
  totalLeaves: number;
...
// ─── Migration: add missing fields to old employee records ───────────────────
function migrateEmployee(e: Partial<Employee>): Employee {
  return {
    ...
    workingDays: e.workingDays ?? [0, 1, 2, 3, 4, 5, 6],
...
const STATUS_DOT: Record<string, string> = {
  ...
  "Late Start Request": "bg-orange-500", "Holiday": "bg-purple-500", "Not Scheduled": "bg-gray-300",
};
...
function getUpcomingBirthdays(employees: Employee[], daysAhead = 7) {
  ...
  }).filter(e => e.daysUntil >= 0 && e.daysUntil <= daysAhead);
}
...
function groupByDept(employees: Employee[]): Record<string, Employee[]> {
  const groups: Record<string, Employee[]> = {};
  [...employees].sort(sortByEmpId).forEach(e => {
...
export default function HRDashboard() {
  ...
  const [empView, setEmpView] = useState<EmpView>("cards");
  ...
  const [holidayWarning, setHolidayWarning] = useState<{ empId: string } | null>(null);
  ...
  // ─── Hydration ──────────────────────────────────────────────────────────────
  useEffect(() => {
    const rawEmps = load<Partial<Employee>[]>("ants_employees", []);
    const migrated = rawEmps.length > 0 ? rawEmps.map(migrateEmployee) : DEFAULT_EMPLOYEES;
  ...
  const filteredAttendanceEmps = useMemo(() => {
  ...
  // Grouped + sorted for attendance table
  ...
  // Is the given date a scheduled working day for this employee?
  const isScheduledDay = (emp: Employee, dateStr: string) => {
    if (emp.employmentType === "full-time") return true;
    const dow = getDay(parseISO(dateStr)); // 0=Sun
    return emp.workingDays.includes(dow);
  };
  ...
  // ─── Attendance handler ──────────────────────────────────────────────────────
  const commitAttendanceEntry = (empId: string, inTime: string, statusOverride?: AttendanceStatus) => {
    ...
    else if (!isNowLeave && wasLeave) setEmployees(prev => prev.map(e => e.id === empId ? { ...e, remainingLeaves: Math.min(e.totalLeaves, e.remainingLeaves + 1) } : e));
  };
  ...
  const handleAttendanceEntry = (empId: string, inTime: string, statusOverride?: AttendanceStatus) => {
    ...
    if (holiday && !holiday.considerAttendance) {
      setPendingEntry({ empId, inTime, status: statusOverride });
      setHolidayWarning({ empId });
      return;
    }
    commitAttendanceEntry(empId, inTime, statusOverride);
  };

  const confirmHolidayOverride = () => {
    if (pendingEntry) commitAttendanceEntry(pendingEntry.empId, pendingEntry.inTime, pendingEntry.status);
    setHolidayWarning(null); setPendingEntry(null);
  };
  ...
  const removeEmployee = (id: string) => {
    if (!confirm("Remove this employee? Attendance records will remain.")) return;
    ...
    reader.onload = (ev) => {
      ...
      text.split("\n").forEach(line => {
        ...
        if (parts[0]?.match(/^\d{4}-\d{2}-\d{2}$/))
          parsed.push({ date: parts[0], name: parts[1] || "Holiday", type: parts[2] === "company" ? "company" : "public", considerAttendance: false });
      });
      if (parsed.length > 0) {
      ...
      } else alert("❌ No valid rows found. Format: YYYY-MM-DD, Holiday Name");
    };
  ...
  const exportCSV = () => {
    ...
    records.forEach(r => {
      ...
      rows.push([fmtDate(r.date), emp?.employeeId || "", emp?.name || "", emp?.designation || "", emp?.department || "", emp?.employmentType || "", emp?.positionType || "", r.status, r.inTime, r.note, r.lateReason]);
    });
    ...
    <div>
      ...
      <div className="flex gap-2">
        <select value={empForm[codeKey] as string} onChange={e => setEmpForm(f => ({ ...f, [codeKey]: e.target.value }))}
          className={`rounded-lg px-2 py-2 text-sm border outline-none focus:ring-2 focus:ring-blue-500 w-24 ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "border-gray-200"}`}>
          <option value="+880">🇧🇩 +880</option>
        </select>
        <input type="tel" value={empForm[numKey] as string} onChange={e => setEmpForm(f => ({ ...f, [numKey]: e.target.value }))}
          placeholder="01XXXXXXXXX"
          className={`flex-1 rounded-lg px-3 py-2 text-sm border outline-none focus:ring-2 focus:ring-blue-500 ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "border-gray-200"}`} />
      </div>
    </div>
  );
  ...
  const CalendarView = () => {
    ...
    const days = getDaysInMonth(calendarMonth), firstDay = getDay(startOfMonth(calendarMonth));
  ...
  const OrgNode = ({ emp, depth }: { emp: Employee; depth: number }) => {
    ...
    return (
      <div className="flex flex-col items-center">
        <div className={`rounded-xl border p-3 w-40 text-center shadow-sm transition-all hover:shadow-md
          ${isInactive ? "opacity-50" : ""}
          ${depth === 0 ? darkMode ? "bg-blue-900/40 border-blue-600" : "bg-blue-50 border-blue-300" :
            depth === 1 ? darkMode ? "bg-gray-700 border-gray-600" : "bg-white border-gray-200" :
          ...
          <div className={`text-xs font-semibold leading-tight ${darkMode ? "text-white" : "text-gray-800"}`}>{emp.name}</div>
          <div className={`text-xs mt-0.5 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{emp.designation || emp.department}</div>
          <div className={`text-xs mt-0.5 ${darkMode ? "text-gray-500" : "text-gray-400"}`}>{emp.employeeId}</div>
        </div>
        {reports.length > 0 && (
          <div className="flex flex-col items-center">
            ...
            <div className="flex gap-4 items-start">
              {reports.map((r, idx) => (
                <div key={r.id} className="flex flex-col items-center">
                  {reports.length > 1 && (
                    <div className={`h-px w-full ${darkMode ? "bg-gray-600" : "bg-gray-300"}`}
                      style={{ marginBottom: -1, width: idx === 0 || idx === reports.length - 1 ? "50%" : "100%",
                        alignSelf: idx === 0 ? "flex-end" : idx === reports.length - 1 ? "flex-start" : "center" }} />
                  )}
  ...
  const OrganogramView = () => {
    ...
    return (
      <div className={`rounded-2xl shadow-lg p-6 overflow-auto ${darkMode ? "bg-gray-800" : "bg-white"}`}>
        <h3 className="text-base font-semibold mb-6 flex items-center gap-2">
          <GitBranch size={16} className="text-blue-500" /> Organisation Chart
        </h3>
        ...
        <p className={`text-xs mt-4 text-center ${darkMode ? "text-gray-500" : "text-gray-400"}`}>
          Set "Reports To" in each employee's profile to build the hierarchy.
        </p>
      </div>
    );
  };
      ...
      <header className={`sticky top-0 z-40 border-b backdrop-blur-sm ${dc ? "bg-gray-900/90 border-gray-700" : "bg-white/90 border-gray-200"}`}>
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={exportCSV} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium ${dc ? "bg-gray-800 hover:bg-gray-700 text-gray-300" : "bg-gray-100 hover:bg-gray-200 text-gray-600"}`}>
              <Download size={13} /> Export CSV
            </button>
            <button onClick={() => setShowHolidayForm(true)} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium ${dc ? "bg-gray-800 hover:bg-gray-700 text-gray-300" : "bg-gray-100 hover:bg-gray-200 text-gray-600"}`}>
              <CalendarOff size={13} /> Holiday
            </button>
            <input ref={csvRef} type="file" accept=".csv" className="hidden" onChange={handleCSVUpload} />
            <button onClick={() => csvRef.current?.click()} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium ${dc ? "bg-gray-800 hover:bg-gray-700 text-gray-300" : "bg-gray-100 hover:bg-gray-200 text-gray-600"}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            <div key={label} className={`rounded-xl p-4 ${bg} flex items-center gap-3`}>
              ...
              <div><div className={`text-2xl font-bold ${color}`}>{value}</div>
                <div className={`text-xs ${dc ? "text-gray-400" : "text-gray-500"}`}>{label}</div></div>
            </div>
          ))}
        </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              <div className="flex items-center gap-3">
                ...
                {/* Toggle cards / organogram */}
                <div className={`flex rounded-lg p-1 gap-1 ${dc ? "bg-gray-800" : "bg-gray-100"}`}>
                  {([["cards", "Cards", LayoutGrid], ["organogram", "Org Chart", GitBranch]] as const).map(([v, label, Icon]) => (
            ...
            {empView === "organogram" ? <OrganogramView /> : (
              // Cards grouped by department, sorted by employee ID
                ...
                {Object.entries(groupByDept(employees)).sort(([a], [b]) => a.localeCompare(b)).map(([dept, emps]) => (
                  <div key={dept}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                      {emps.map(emp => {
                        const isInactive = !!emp.cessationDate;
                        const reportsToEmp = employees.find(e => e.id === emp.reportsTo);
                        return (
                          <div key={emp.id} className={`rounded-2xl shadow-md p-5 border transition-all hover:shadow-lg
                            ${isInactive ? dc ? "bg-gray-800/50 border-gray-700 opacity-60" : "bg-gray-50 border-gray-200 opacity-70"
                              : dc ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"}`}>
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold text-white ${isInactive ? "bg-gray-400" : "bg-blue-600"}`}>
                                  {emp.name.split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase()}
                                </div>
                                <div>
                                  <div className="font-semibold text-sm">{emp.name}</div>
                                  <div className={`text-xs ${dc ? "text-gray-400" : "text-gray-500"}`}>{emp.employeeId}</div>
                                </div>
                              </div>
                              <div className="flex gap-1 items-center">
                                {isInactive && <span className="text-xs px-2 py-0.5 rounded-full bg-gray-200 text-gray-600">Inactive</span>}
                                <button onClick={() => openEditEmp(emp)} className={`p-1.5 rounded-lg ${dc ? "hover:bg-gray-700 text-gray-400" : "hover:bg-gray-100 text-gray-500"}`}><Edit2 size={14} /></button>
                                <button onClick={() => removeEmployee(emp.id)} className={`p-1.5 rounded-lg ${dc ? "hover:bg-rose-900/30 text-gray-600 hover:text-rose-400" : "hover:bg-rose-50 text-gray-300 hover:text-rose-500"}`}><Trash2 size={14} /></button>
                              </div>
                            </div>
                            <div className="space-y-1.5 text-xs">
                              <div className="flex items-center gap-2">
                                <Briefcase size={11} className="text-blue-400 shrink-0" />
                                <span className={dc ? "text-gray-300" : "text-gray-700"}>{emp.designation || "—"}</span>
                              </div>
                              <div className="flex gap-2 flex-wrap">
                                <span className={`px-2 py-0.5 rounded-full font-medium ${emp.employmentType === "part-time" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}>
                                  {emp.employmentType === "part-time" ? "Part-time" : "Full-time"}
                                </span>
                                <span className={`px-2 py-0.5 rounded-full font-medium ${emp.positionType === "contractual" ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700"}`}>
                                  {emp.positionType === "contractual" ? "Contractual" : "Permanent"}
                                </span>
                              </div>
                              {emp.employmentType === "part-time" && (
                                <div className={`text-xs ${dc ? "text-gray-400" : "text-gray-500"}`}>
                                  Days: {emp.workingDays.map(d => DAYS_SHORT[d]).join(", ")}
                                  {emp.workingHours && ` · ${emp.workingHours}`}
                                </div>
                              )}
                              {emp.companyEmail && <div className="flex items-center gap-2"><Mail size={11} className="text-blue-400 shrink-0" /><span className={`truncate ${dc ? "text-gray-300" : "text-gray-600"}`}>{emp.companyEmail}</span></div>}
                              {emp.phone && <div className="flex items-center gap-2"><Phone size={11} className="text-emerald-400 shrink-0" /><span className={dc ? "text-gray-300" : "text-gray-600"}>{emp.phoneCode} {emp.phone}</span></div>}
                              {emp.presentAddress && <div className="flex items-center gap-2"><MapPin size={11} className="text-rose-400 shrink-0" /><span className={`truncate ${dc ? "text-gray-300" : "text-gray-600"}`}>{emp.presentAddress}</span></div>}
                              <div className="flex items-center gap-3 pt-0.5">
                                {emp.bloodGroup && <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full font-medium ${dc ? "bg-rose-900/30 text-rose-300" : "bg-rose-50 text-rose-600"}`}><Droplets size={10} />{emp.bloodGroup}</span>}
                                {emp.birthday && <span className={`flex items-center gap-1 ${dc ? "text-gray-400" : "text-gray-500"}`}><Cake size={10} />{fmtDate(emp.birthday)}</span>}
                              </div>
                              <div className={`pt-1 border-t ${dc ? "border-gray-700 text-gray-400" : "border-gray-100 text-gray-500"} flex flex-wrap gap-x-3`}>
                                <span>Joined: {emp.joiningDate ? fmtDate(emp.joiningDate) : "—"}</span>
                                {emp.cessationDate && <span className="text-rose-500">Left: {fmtDate(emp.cessationDate)}</span>}
                                {reportsToEmp && <span>Reports to: {reportsToEmp.name}</span>}
                              </div>
                              <div className={dc ? "text-gray-400" : "text-gray-500"}>Leaves: <strong>{emp.remainingLeaves}</strong>/{emp.totalLeaves}</div>
                            </div>
                          </div>
                        );
                      })}
        ...
        {mainTab === "attendance" && (
          <div className="space-y-5">
            <div className={`flex rounded-lg p-1 gap-1 w-fit ${dc ? "bg-gray-800" : "bg-gray-100"}`}>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {Object.entries(groupByDept(employees)).sort(([a], [b]) => a.localeCompare(b)).flatMap(([dept, emps]) => [
                        <tr key={`dept-${dept}`}>
                          <td colSpan={9} className={`py-2 px-0 text-xs font-bold uppercase tracking-wider ${dc ? "text-gray-500" : "text-gray-400"}`}>{dept}</td>
                        </tr>,
                        ...emps.map(emp => {
              ...
              <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
                      ...
                      <tbody>
                        {Object.entries(attendanceGroups).sort(([a], [b]) => a.localeCompare(b)).flatMap(([dept, emps]) => [
                          ...
                          ...emps.map(emp => {
                            ...
                            const scheduled = isScheduledDay(emp, selectedDate);
                            return (
                              <tr key={emp.id} className={`border-b last:border-0 ${dc ? "border-gray-700" : "border-gray-50"} ${!scheduled ? "opacity-60" : ""}`}>
                                <td className="py-3">
                                ...
                                <td className="py-3">
                                  <div className="flex items-center justify-center gap-1">
                                    <button onClick={() => handleAttendanceEntry(emp.id, "00:00", "WFH")} title="WFH" className={`p-1.5 rounded-lg hover:scale-110 transition-all ${dc ? "hover:bg-sky-900/40 text-sky-400" : "hover:bg-sky-50 text-sky-600"}`}><Home size={15} /></button>
                                    <button onClick={() => handleAttendanceEntry(emp.id, "00:00", "Leave")} title="Leave" className={`p-1.5 rounded-lg hover:scale-110 transition-all ${dc ? "hover:bg-amber-900/40 text-amber-400" : "hover:bg-amber-50 text-amber-600"}`}><UserX size={15} /></button>
                                    <button onClick={() => handleAttendanceEntry(emp.id, "00:00", "Absent")} title="Absent" className={`p-1.5 rounded-lg hover:scale-110 transition-all ${dc ? "hover:bg-gray-700 text-gray-400" : "hover:bg-gray-100 text-gray-500"}`}><XCircle size={15} /></button>
                                    <button onClick={() => handleAttendanceEntry(emp.id, "00:00", "Late Start Request")} title="Late Start" className={`p-1.5 rounded-lg hover:scale-110 transition-all ${dc ? "hover:bg-orange-900/40 text-orange-400" : "hover:bg-orange-50 text-orange-600"}`}><Coffee size={15} /></button>
                                  </div>
                                </td>
                                <td className="py-3 text-center">
                                  <div className="flex items-center justify-center gap-1">
                ...
                <div className="space-y-4">
                  ...
                  <div className={`rounded-2xl shadow-lg p-5 ${dc ? "bg-gray-800" : "bg-white"}`}>
                    ...
                    {holidays.length === 0 && <p className={`text-xs ${dc ? "text-gray-500" : "text-gray-400"}`}>No holidays added. Use Import CSV or Add Holiday.</p>}
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className={`text-xs font-medium block mb-1 ${dc ? "text-gray-400" : "text-gray-500"}`}>Reports To</label>
                    <select value={empForm.reportsTo} onChange={e => setEmpForm(f => ({ ...f, reportsTo: e.target.value }))}
                      className={`w-full rounded-lg px-3 py-2 text-sm border outline-none focus:ring-2 focus:ring-blue-500 ${dc ? "bg-gray-700 border-gray-600 text-white" : "border-gray-200"}`}>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className={`text-xs font-medium block mb-1 ${dc ? "text-gray-400" : "text-gray-500"}`}>Employment Type</label>
                    <select value={empForm.employmentType} onChange={e => setEmpForm(f => ({ ...f, employmentType: e.target.value as EmploymentType }))}
                      className={`w-full rounded-lg px-3 py-2 text-sm border outline-none focus:ring-2 focus:ring-blue-500 ${dc ? "bg-gray-700 border-gray-600 text-white" : "border-gray-200"}`}>
                  ...
                  <div>
                    <label className={`text-xs font-medium block mb-1 ${dc ? "text-gray-400" : "text-gray-500"}`}>Position Type</label>
                    <select value={empForm.positionType} onChange={e => setEmpForm(f => ({ ...f, positionType: e.target.value as PositionType }))}
                      className={`w-full rounded-lg px-3 py-2 text-sm border outline-none focus:ring-2 focus:ring-blue-500 ${dc ? "bg-gray-700 border-gray-600 text-white" : "border-gray-200"}`}>
                ...
                {empForm.employmentType === "part-time" && (
                  <div className={`mt-4 p-4 rounded-xl border ${dc ? "bg-gray-700/50 border-gray-600" : "bg-amber-50 border-amber-200"}`}>
                    ...
                    <div className="mb-3">
                      <label className={`text-xs font-medium block mb-2 ${dc ? "text-gray-400" : "text-gray-500"}`}>Working Days</label>
                      <div className="flex flex-wrap gap-2">
                        {DAYS_OF_WEEK.map((day, idx) => {
                          const checked = empForm.workingDays.includes(idx);
                          return (
                            <button key={day} type="button"
                              onClick={() => setEmpForm(f => ({
                                ...f, workingDays: checked
                    ...
                    <div>
                      <label className={`text-xs font-medium block mb-1 ${dc ? "text-gray-400" : "text-gray-500"}`}>Working Hours (free text)</label>
                      <input type="text" value={empForm.workingHours} onChange={e => setEmpForm(f => ({ ...f, workingHours: e.target.value }))}
                        placeholder="e.g. 10:00-14:00 or 4 hours/day"
                        className={`w-full rounded-lg px-3 py-2 text-sm border outline-none focus:ring-2 focus:ring-blue-500 ${dc ? "bg-gray-700 border-gray-600 text-white" : "border-gray-200"}`} />
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                  <div>
                    <label className={`text-xs font-medium block mb-1 ${dc ? "text-gray-400" : "text-gray-500"}`}>Total Leaves / Year</label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className={`rounded-2xl shadow-2xl w-full max-w-sm ${dc ? "bg-gray-800" : "bg-white"}`}>
            ...
            <div className="px-6 pb-4 space-y-3">
              <div>
                <label className={`text-xs font-medium block mb-1 ${dc ? "text-gray-400" : "text-gray-500"}`}>Date *</label>
                <input type="date" value={newHoliday.date} onChange={e => setNewHoliday(h => ({ ...h, date: e.target.value }))}
                  className={`w-full rounded-lg px-3 py-2 text-sm border outline-none focus:ring-2 focus:ring-purple-500 ${dc ? "bg-gray-700 border-gray-600 text-white" : "border-gray-200"}`} />
              </div>
              <div>
                <label className={`text-xs font-medium block mb-1 ${dc ? "text-gray-400" : "text-gray-500"}`}>Holiday Name *</label>
                <input value={newHoliday.name} onChange={e => setNewHoliday(h => ({ ...h, name: e.target.value }))} placeholder="e.g. Eid ul-Fitr"
                  className={`w-full rounded-lg px-3 py-2 text-sm border outline-none focus:ring-2 focus:ring-purple-500 ${dc ? "bg-gray-700 border-gray-600 text-white" : "border-gray-200"}`} />
              </div>
              <div>
                <label className={`text-xs font-medium block mb-1 ${dc ? "text-gray-400" : "text-gray-500"}`}>Type</label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  ...
                  <div className={`text-xs ${dc ? "text-gray-400" : "text-gray-500"}`}>Won't deduct from leave balance. Off = shows warning before marking.</div>
                </div>
              </label>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className={`rounded-2xl shadow-2xl w-full max-w-sm p-6 ${dc ? "bg-gray-800" : "bg-white"}`}>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setHolidayWarning(null); setPendingEntry(null); }} className={`flex-1 py-2 rounded-lg text-sm font-medium ${dc ? "bg-gray-700 hover:bg-gray-600" : "bg-gray-100 hover:bg-gray-200"}`}>Cancel</button>
